...
package Text::Textile2;

use strict;
use Exporter;
@Text::Textile2::ISA = qw(Exporter);
...
sub textile {
    ...
    my @para = split /(\n{2,})/, $str;
    my (
        $block, $bqlang, $filter,     $class, $sticky,
    ...
    foreach my $para (@para) {
        ...
        if ( $buffer =~ m/$blocktags/ ) {
            ...
            $buffer = $self->format_block( text => "|$filter|" . $buffer, inline => 1 )
                if defined $filter;
            $out .= $pre . $buffer . $post;
        }
    }
...
sub format_list {
    ...
    foreach my $line (@lines) {
            ...
            )
        {
            ...
            my $itemparam  = $3;
            $line = $4;
            my ( $blockclsty, $blockalign, $blockattr, $itemattr, $itemclsty, $itemalign );
...
sub format_image {
    ...
    if ( defined $align ) {
        if ( $self->{css_mode} ) {
            ...
            if ($alignment) {
                my $imgvalign =
                    ( $alignment =~ m/(top|bottom)/ ? 'text-' . $alignment : $alignment );
...
sub format_table {
    ...
    foreach my $row (@rows) {
        ...
        if ( $colspan > 1 ) {
            ...
            $colspan--;
            $row_out =
                qq{<td} . ( $colspan > 1 ? qq{ colspan="$colspan"} : '' ) . qq{></td>$row_out};
        }
...
sub _tokenize {
    ...
    my $depth = 6;
    my $nested_tags =
        join( '|', ('(?:</?[A-Za-z0-9:]+ \s? (?:[^<>]') x $depth ) . ( ')*>)' x $depth );
...
Text::Textile2 - A humane web text generator.

=head1 SYNOPSIS

    use Text::Textile2 qw(textile);
    my $text = <<EOT;
    h1. Heading
    ...
    * Three
    ...
    # OOP usage
    my $textile = new Text::Textile2;
    $html = $textile->process($text);
    print $html;

=head1 ABSTRACT

Text::Textile2 is a Perl-based implementation of Dean Allen's Textile
...
Instantiates a new Text::Textile2 object. Optional options
...
Assigns the HTML flavor of output from Text::Textile2. Currently
...
Text::Textile2. The following key names for such a hash are
recognized:
...
At this time, Text::Textile2 only changes its behavior
...
Text::Textile2 to use. Returns a hash reference of the currently
assigned filters.
...
Text::Textile2 processes text in units of blocks and lines.
...
Text::Textile2 was written by Brad Choate, brad@bradchoate.com.
